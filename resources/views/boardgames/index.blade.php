<!DOCTYPE html>
<!--[if IE 8 ]><html class="ie ie8" lang="en"> <![endif]-->
<!--[if IE 9 ]><html class="ie ie9" lang="en"> <![endif]-->
<!--[if (gte IE 9)|!(IE)]><!--><html lang="en"> <!--<![endif]-->
<head>
    <meta charset="utf-8">
    <meta name="description" content="Мир Настольных Игр">
    <meta name="author" content="tatwerat.com">
    <!--mobile-meta-->
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
    <title>Мир Настольных Игр</title>
    <!--owl-carousel-->
    <link rel="stylesheet" href="{{ url('js/owl-carousel/owl.carousel.css') }}" type="text/css">
    <link rel="stylesheet" href="{{ url('js/owl-carousel/owl.theme.css') }}" type="text/css">
    <link rel="stylesheet" href="{{ url('js/owl-carousel/owl.transitions.css') }}" type="text/css">
    <!--color-box-->
    <link rel="stylesheet" href="{{ url('js/colorbox/colorbox.min.css') }}" type="text/css">
    <!--main-style-->
    <link rel="stylesheet" media="screen" href="{{ url('css/style.css') }}" type="text/css">
    <!--blog-style-->
    <link rel="stylesheet" class="blog-style" media="screen" href="{{ url('css/blog.css') }}" type="text/css">
    <!--responsive-style-->
    <link rel="stylesheet" href="{{ url('css/responsive.css') }}" type="text/css">
</head>
<body data-spy="scroll" data-target=".AH_Nav" data-offset="300" data-page="home">
<div id="Wrapper">
    <header>
        <div class="container-fluid" id="Header">
            <div class="container">
                <div class="logo"><span>М</span></div>
                {{--<div class="logo"><img src="images/logo.png" alt=""></div>--}}
                <p class="logoName">Мир Настольных Игр</p>
                <p class="jobTitle">Знакомство с Лучшими в Мире Настольными Играми</p>
                <p class="jobTitle">0 (793) 1-60-60</p>
                <a href="#About" class="moreData fa fa-angle-down"></a>
            </div>
        </div>
    </header>
    <nav class="navbar navbar-default AH_Nav" id="navTopMenu" role="navigation">
        <div class="container-fluid">
            <div class="container">
                <div class="navbar-header">
                    <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#navbar-collapse">
                        <i class="fa fa-bars"></i>
                    </button>
                </div>
                <div class="collapse navbar-collapse bs-example-js-navbar-scrollspy" id="navbar-collapse">
                    <ul class="nav navbar-nav">
                        <li class="active">
                            <a href="#Header" class="home" id="homepages" data-hover="dropdown">ГЛАВНАЯ</a>
                            {{--<ul class="dropdown-menu"  aria-labelledby="homepages">
                                <li><a href="index.html">Home Min Header</a></li>
                                <li><a href="index-fullheight.html">Home Full Header</a></li>
                                <li><a href="index-boxed.html">Home Boxed Width</a></li>
                            </ul>--}}
                        </li>
                        <li><a href="#About" class="about">О НАС</a></li>
                        <li><a href="#Portfolio" class="portfolio">ИГРЫ</a></li>
                        <li><a href="#Activity" class="activity">СТАТЬИ</a></li>
                        <li><a href="#Clients" class="clients">КЛИЕНТЫ</a></li>
                        <li><a href="#ContactUS" class="contact">СВЯЗЬ</a></li>
                        {{--<li class="active dropdown">
                            <a href="blog.html" id="bloglist" data-hover="dropdown" class="contact">Pages &raquo;</a>
                            <ul class="dropdown-menu"  aria-labelledby="bloglist">
                                <li><a href="blog.html">blog</a></li>
                                <li><a href="single-blog.html">single post</a></li>
                                <li><a href="portfolio.html">portfolio</a></li>
                                <li><a href="portfolio-fullwidth-columns2.html">portfolio columns 2</a></li>
                                <li><a href="portfolio-fullwidth-columns3.html">portfolio columns 3</a></li>
                                <li><a href="portfolio-fullwidth-columns4.html">portfolio columns 4</a></li>
                            </ul>
                        </li>--}}
                        {{--<li class="dropdown">
                            <a href="Shop.html" id="shoplist" data-hover="dropdown" class="contact">Shop &raquo;</a>
                            <ul class="dropdown-menu"  aria-labelledby="shoplist">
                                <li><a href="shop.html">shop [1]</a></li>
                                <li><a href="shop-2.html">shop [2]</a></li>
                                <li><a href="product-single.html">product single</a></li>
                                <li><a href="cart.html">cart</a></li>
                            </ul>
                        </li>--}}
                    </ul>
                </div>
            </div>
            <div class="scrollTop fa fa-caret-up"></div>
        </div>
    </nav>
    <div id="Body">
        <div class="container-fluid" id="About">
            <div class="container">
                <div class="row">
                    <div class="col-md-3 col-sm-6">
                        <div class="profileBlock">
                            <a href="" class="img"><img src="images/logo.png" alt=""></a>
                            <h4>Путеводитель</h4>
                            <p><span class="icon-location-8"></span>В Мир Настольных Игр</p>
                            <div class="socialIcons">
                                <ul class="row">
                                    <li class="col-xs-3 facebook">
                                        <a href="https://www.facebook.com/mirigr.md" target="_blank" class="n_title" title="facebook"><i class="fa fa-facebook"></i></a>
                                    </li>
                                    {{--<li class="col-xs-3 linkedin">
                                        <a href="#" target="_blank" class="n_title" title="linkedin"><i class="fa fa-linkedin"></i></a>
                                    </li>--}}
                                    {{--<li class="col-xs-3 twitter">
                                        <a href="#" target="_blank" class="n_title" title="twitter"><i class="fa fa-twitter"></i></a>
                                    </li>--}}
                                    {{--<li class="col-xs-3 youtube">
                                        <a href="https://www.youtube.com/watch?v=sz-R78_VRqk" target="_blank" class="n_title" title="behance"><i class="fa fa-behance fa-3"></i></a>
                                    </li>--}}
                                </ul>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4 col-sm-6">
                        <div class="flickrSlider">
                            <div class="Slider owl-carousel">
                                <div class="slide"><a href="" class="view"></a><img src="images/flickr/flickr1.gif" alt=""></div>
                                <div class="slide"><a href="" class="view"></a><img src="images/flickr/flickr2.gif" alt=""></div>
                                <div class="slide"><a href="" class="view"></a><img src="images/flickr/flickr3.gif" alt=""></div>
                                <div class="slide"><a href="" class="view"></a><img src="images/flickr/flickr4.gif" alt=""></div>
                                <div class="slide"><a href="" class="view"></a><img src="images/flickr/flickr5.gif" alt=""></div>
                            </div>
                            <h3>flickr</h3>
                            <div class="sliderPager"></div>
                            <a href="" class="likes"><span class="fa fa-heart"></span> 514</a>
                            <a href="" class="viewAll fa fa-search"></a>
                        </div>
                    </div>
                    <div class="col-md-5 col-sm-12">
                        <div class="aboutMe">
                            <img src="{{ url('images/posts/about/about_header.jpg') }}" alt="">
                            <p><b>НОВИНКА В МОЛДОВЕ</b> : Знакомство с лучшими в Мире Настольными Играми в кругу Семьи и Друзей!
                                    Откройте новый горизонт альтернативного вида совместного досуга в кругу близких... <br> <a href="{{ route('post.view', $post->id) }}">[Читать Дальше]</a>
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!--/About-->
        <div class="container-fluid" id="Skills">
            <div class="container">
                <div class="row">
                    <div class="col-md-3 col-sm-6">
                        <div class="skill">
                            <h2>Экономические</h2>
                            <input type="text"
                                   disabled="disabled"
                                   class="skillRang"
                                   data-width="160"
                                   data-min="0"
                                   data-max="100"
                                   data-cursor=false
                                   data-thickness=".03"
                                   data-fgColor="#ff7f66"
                                   data-bgColor="#ddd"
                                   data-displayInput="true"
                                   data-readOnly=true
                                   value="99">
                        </div>
                    </div>
                    <div class="col-md-3 col-sm-6">
                        <div class="skill">
                            <h2>Семейные</h2>
                            <input type="text"
                                   disabled="disabled"
                                   class="skillRang"
                                   data-width="160"
                                   data-min="0"
                                   data-max="100"
                                   data-cursor=false
                                   data-thickness=".03"
                                   data-fgColor="#ff7f66"
                                   data-bgColor="#ddd"
                                   data-displayInput="true"
                                   data-readOnly=true
                                   value="95">
                        </div>
                    </div>
                    <div class="col-md-3 col-sm-6">
                        <div class="skill">
                            <h2>Стратегические</h2>
                            <input type="text"
                                   disabled="disabled"
                                   class="skillRang"
                                   data-width="160"
                                   data-min="0"
                                   data-max="100"
                                   data-cursor=false
                                   data-thickness=".03"
                                   data-fgColor="#ff7f66"
                                   data-bgColor="#ddd"
                                   data-displayInput="true"
                                   data-readOnly=true
                                   value="90">
                        </div>
                    </div>
                    <div class="col-md-3 col-sm-6">
                        <div class="skill">
                            <h2>Абстрактные</h2>
                            <input type="text"
                                   disabled="disabled"
                                   class="skillRang"
                                   data-width="160"
                                   data-min="0"
                                   data-max="100"
                                   data-cursor=false
                                   data-thickness=".03"
                                   data-fgColor="#ff7f66"
                                   data-bgColor="#ddd"
                                   data-displayInput="true"
                                   data-readOnly=true
                                   value="85">
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!--/Skills-->
        <div class="container-fluid" id="Portfolio">
            <div class="container">
                <h2>Список Игр <a href="" class="next fa fa-angle-right"></a><a href="" class="prev fa fa-angle-left"></a></h2>
                <div class="Slider owl-carousel">
                    <div class="work">
                        <a href="images/projects/project1-lg.gif" title="your project name [1]"><i class="fa fa-link"></i> Your Project Name [1]</a>
                        <img src="images/projects/project1.gif" alt="">
                    </div>
                    <div class="work">
                        <a href="images/projects/project2-lg.gif" title=" your project name [2]"><i class="fa fa-link"></i> Your Project Name [2]</a>
                        <img src="images/projects/project2.gif" alt="">
                    </div>
                    <div class="work">
                        <a href="images/projects/project3-lg.gif" title=" your project name [3]"><i class="fa fa-link"></i> Your Project Name [3]</a>
                        <img src="images/projects/project3.gif" alt="">
                    </div>
                    <div class="work">
                        <a href="images/projects/project4-lg.gif" title=" your project name [4]"><i class="fa fa-link"></i> Your Project Name [4]</a>
                        <img src="images/projects/project4.gif" alt="">
                    </div>
                    <div class="work">
                        <a href="images/projects/project5-lg.gif" title=" your project name [5]"><i class="fa fa-link"></i> Your Project Name [5]</a>
                        <img src="images/projects/project5.gif" alt="">
                    </div>
                    <div class="work">
                        <a href="images/projects/project6-lg.gif" title=" your project name [6]"><i class="fa fa-link"></i> Your Project Name [6]</a>
                        <img src="images/projects/project6.gif" alt="">
                    </div>
                </div>
            </div>
        </div>
        <!--/Portfolio-->
        <div class="container-fluid" id="Activity">
            <div class="container">
                <h2>Статьи</h2>
                <div class="row">
                    <div class="col-md-3 col-sm-6">
                        <div class="post">
                            <img src="images/posts/post1.gif" alt="">
                            <a href="single-blog.html" class="link fa fa-link"></a>
                            <section>
                                <h3><a href="single-blog.html">At vero eos et accusam et justo duo dolores.</a></h3>
                                <aside class="text">orem ipsum dolor sit amet, consectetur adipiscing elit [..] </aside>
                                <p><i class="fa fa-clock-o"></i> 25-6-2014 <i class="fa fa-comments"></i> 3 comments</p>
                            </section>
                        </div>
                    </div>
                    <div class="col-md-3 col-sm-6">
                        <div class="post">
                            <img src="images/posts/post2.gif" alt="">
                            <a href="single-blog.html" class="link fa fa-link"></a>
                            <section>
                                <h3><a href="single-blog.html">At vero eos et accusam et justo duo dolores.</a></h3>
                                <aside class="text">orem ipsum dolor sit amet, consectetur adipiscing elit [..] </aside>
                                <p><i class="fa fa-clock-o"></i> 25-6-2014 <i class="fa fa-comments"></i> 3 comments</p>
                            </section>
                        </div>
                    </div>
                    <div class="col-md-3 col-sm-6">
                        <div class="post">
                            <img src="images/posts/post3.gif" alt="">
                            <a href="single-blog.html" class="link fa fa-link"></a>
                            <section>
                                <h3><a href="single-blog.html">At vero eos et accusam et justo duo dolores.</a></h3>
                                <aside class="text">orem ipsum dolor sit amet, consectetur adipiscing elit [..] </aside>
                                <p><i class="fa fa-clock-o"></i> 25-6-2014 <i class="fa fa-comments"></i> 3 comments</p>
                            </section>
                        </div>
                    </div>
                    <div class="col-md-3 col-sm-6">
                        <div class="post">
                            <img src="images/posts/post4.gif" alt="">
                            <a href="single-blog.html" class="link fa fa-link"></a>
                            <section>
                                <h3><a href="single-blog.html">At vero eos et accusam et justo duo dolores.</a></h3>
                                <aside class="text">orem ipsum dolor sit amet, consectetur adipiscing elit [..] </aside>
                                <p><i class="fa fa-clock-o"></i> 25-6-2014 <i class="fa fa-comments"></i> 3 comments</p>
                            </section>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-2 col-md-offset-5">
                        <a href="blog.html" class="btn btn-block viewAll">view more</a>
                    </div>
                </div>
            </div>
        </div>
        <!--/Activity-->
        <div class="container-fluid" id="Clients">
            <div class="container">
                <h2>OUR CLIENTS</h2>
                <div class="Slider owl-carousel">
                    <div class="client">
                        <a href=""><img src="{{ url('images/clients/1.gif') }}" alt=""></a>
                    </div>
                    <div class="client">
                        <a href=""><img src="{{ url('images/clients/2.gif') }}" alt=""></a>
                    </div>
                    <div class="client">
                        <a href=""><img src="{{ url('images/clients/3.gif') }}" alt=""></a>
                    </div>
                    <div class="client">
                        <a href=""><img src="{{ url('images/clients/4.gif') }}" alt=""></a>
                    </div>
                    <div class="client">
                        <a href=""><img src="{{ url('images/clients/5.gif') }}" alt=""></a>
                    </div>
                    <div class="client">
                        <a href=""><img src="{{ url('images/clients/6.gif') }}" alt=""></a>
                    </div>
                    <div class="client">
                        <a href=""><img src="{{ url('images/clients/7.gif') }}" alt=""></a>
                    </div>
                    <div class="client">
                        <a href=""><img src="{{ url('images/clients/8.gif') }}" alt=""></a>
                    </div>
                </div>
            </div>
        </div>
        <!--/Clients-->
        <div class="container-fluid" id="ContactUS">
            <div class="container">
                <h2>Скорая Связь</h2>
                <div class="row">
                    <div class="col-md-4 col-sm-4">
                        <div class="contactItem">
                            <i class="fa fa-map-marker"></i>
                            <p>Молдова, Кишинев</p>
                        </div>
                    </div>
                    <div class="col-md-4 col-sm-4">
                        <div class="contactItem">
                            <i class="fa fa-envelope"></i>
                            <p>amoscaliuc@gmail.com</p>
                        </div>
                    </div>
                    <div class="col-md-4 col-sm-4">
                        <div class="contactItem">
                            <i class="fa fa-mobile"></i>
                            <p>+373 793 1-60-60</p>
                        </div>
                    </div>
                </div>
                <form action="#" method="post">
                    <div class="row">
                        <div class="col-md-4 col-sm-4 col-xs-4">
                            <input type="text" name="uname" class="form-control" placeholder="Полное Имя" required="required">
                        </div>
                        <div class="col-md-4 col-sm-4 col-xs-4">
                            <input type="email" name="uemail" class="form-control" placeholder="Ваш Имейл" required="required">
                        </div>
                        <div class="col-md-4 col-sm-4 col-xs-4">
                            <input type="text" name="uphone" class="form-control" placeholder="Номер Телефона" required="required">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                            <textarea name="umsg" class="form-control" rows="6" placeholder="Ваше Сообщение" required="required"></textarea>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                            <button type="submit" class="btn btn-danger sendContact">Отправить Сообщение</button>
                            <p class="pull-right">Мы ответим Вам в течении 1 часа</p>
                        </div>
                    </div>
                </form>
            </div>
        </div>
        <!--/ContactUS-->
        <div class="container-fluid fellowUs">
            <div class="container">
                <ul class="col-md-8 col-md-offset-2">
                    <li><a href="https://www.facebook.com/mirigr.md" target="_blank" class="fa fa-facebook"></a></li>
                    {{--<li><a href="" class="fa fa-twitter"></a></li>--}}
                    <li><a href="https://www.facebook.com/mirigr.md" target="_blank" class="fa fa-google-plus"></a></li>
                    <li><a href="https://www.facebook.com/mirigr.md" target="_blank" class="fa fa-linkedin"></a></li>
                    <li><a href="https://www.youtube.com/watch?v=sz-R78_VRqk" target="_blank" class="fa fa-youtube"></a></li>
                    {{--<li><a href="" class="fa fa-dribbble"></a></li>
                    <li><a href="" class="fa fa-behance"></a></li>
                    <li><a href="" class="fa fa-android"></a></li>
                    <li><a href="" class="fa fa-apple"></a></li>--}}
                    <li><a href="" class="fa fa-rss"></a></li>
                </ul>
            </div>
        </div>
        <!--/fellowUs-->
    </div>
    <!--/Body-->
</div>
<!--/Wrapper-->
<div class="wrapperLoading"><span class="fa fa-refresh"></span></div>
<!--java scripts file-->
<script type="text/javascript" src="{{ url('./js/jquery/jquery.js') }}"></script>
<script type='text/javascript' src="{{ url('./js/drag-drop/drag-drop.js') }}"></script>
<script type="text/javascript" src="{{ url('./js/bootstrap/bootstrap.min.js') }}"></script>
<script type="text/javascript" src="{{ url('./js/colorbox/colorbox.min.js') }}"></script>
<script type="text/javascript" src="{{ url('./js/nicescroll/jquery.nicescroll.min.js') }}"></script>
<script type="text/javascript" src="{{ url('./js/knob/jquery.knob.js') }}"></script>
<script type="text/javascript" src="{{ url('./js/owl-carousel/owl.carousel.min.js') }}"></script>
<script type="text/javascript" src="{{ url('./js/viewport-checker/viewportchecker.js') }}"></script>
<script type="text/javascript" src="{{ url('./js/jquery/jquery.custom.js') }}"></script>
<!--/java scripts file-->
</body>
</html>
